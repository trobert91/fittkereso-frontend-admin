...
export function TaskTable() {
  ...
  const [queuesFilter, setQueuesFilter] = useState<QueueName[]>([]);
  const [selectedTask, setSelectedTask] = useState<Task | null>(null);
  const [detailsOpened, { open: openDetails, close: closeDetails }] = useDisclosure(false);

  const [sorting, setSorting] = useState<SortingState>([{ id: "createdAt", desc: true }]);

  const { search, loading, searchResult } = useTaskSearch();
    ...
    () => [
      ...
      columnHelper.accessor("executionTimeInSec", {
        ...
        cell: (props) => {
          ...
          return <Text size="sm">{value != null ? value.toFixed(2) : "-"}</Text>;
        },
      }),
  ...
  return (
    <>
      ...
      >
        {selectedTask && (
          <Stack gap="xs">
            <Text size="sm"><strong>ID:</strong> {selectedTask.id}</Text>
            <Text size="sm"><strong>Queue:</strong> {selectedTask.queue}</Text>
            <Text size="sm"><strong>Status:</strong> {selectedTask.status}</Text>
            <Text size="sm"><strong>Attempts:</strong> {selectedTask.attempts}</Text>
            <Text size="sm"><strong>Exec Time (s):</strong> {selectedTask.executionTimeInSec != null ? selectedTask.executionTimeInSec.toFixed(2) : "-"}</Text>
            <Text size="sm"><strong>Delete After Success:</strong> {selectedTask.deleteAfterSuccess ? "yes" : "no"}</Text>
            <Text size="sm"><strong>Scheduled At:</strong> {formatDate(selectedTask.scheduledAt)}</Text>
            <Text size="sm"><strong>Last Run At:</strong> {formatDate(selectedTask.lastRunAt)}</Text>
            <Text size="sm"><strong>Locked At:</strong> {formatDate(selectedTask.lockedAt)}</Text>
            <Text size="sm"><strong>Created At:</strong> {formatDate(selectedTask.createdAt)}</Text>
            <Text size="sm"><strong>Updated At:</strong> {formatDate(selectedTask.updatedAt)}</Text>
            <Text size="sm" fw={500} mt="sm">Payload:</Text>
          ...
          <Card shadow="sm" padding="sm" radius="sm" mb="xl" withBorder>
            <Group gap="md">
              ...
              <MultiSelect
                ...
                data={Object.values(QueueName).map((q) => ({
                  value: q,
                  label: q,
                }))}
                ...
                onChange={(vals) => {
                  setPage(1);
                  setQueuesFilter(vals as unknown as QueueName[]);
                }}
                clearable
                searchable
                maw={300}
